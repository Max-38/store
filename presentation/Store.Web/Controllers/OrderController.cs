...
namespace Store.Web.Controllers
{
    public class OrderController : Controller
    {
        ...
        public IActionResult SendConfirmationCode(int id, string cellPhone)
        {
            ...
            {
                model.Errors["cellPhone"] = "номер телефона не соответствует формату +79256356519";
                return View("Index", model);
            }

            int code = 1111; // randome.Next(1000, 10000)
        ...
        public IActionResult Confirmate(int id, string cellPhone, int code)
        {
            ...
            {
                OrderId = id,
                Methods = deliveryServices.ToDictionary(services => services.UniqueCode,
        ...
        public IActionResult NextDelivery(int id, string uniqueCode, int step, Dictionary<string, string> values) 
        {
            ...
            if (form.IsFinal)
            {
                ...
                {
                    OrderId = id,
                    Methods = paymentServices.ToDictionary(services => services.UniqueCode,
                                                      services => services.Title)
                };
